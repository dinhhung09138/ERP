...
namespace Service.Common
{
    public class MaritalStatusService : BaseService, IMaritalStatusService
    {
        ...
        private readonly string CacheKey = "MaritalStatus_data";
        ...
        public async Task<ResponseModel> GetList(FilterModel filter)
        {
            ...
            try
            {
                var query = from m in _context.MaritalStatusRepository.Query()
                            where !m.Deleted
                ...
                BaseListModel<MaritalStatusModel> listItems = new BaseListModel<MaritalStatusModel>();
                listItems.TotalItems = await _context.MaritalStatusRepository.Query().Where(m => !m.Deleted).CountAsync();
        ...
        public async Task<ResponseModel> DropDownSelection()
        {
            ...
            try
            {
                var cacheData = _memoryCachingService.GetList<MaritalStatusModel>(CacheKey);
                ...
                else
                {
                    ...
                    _memoryCachingService.Set<MaritalStatusModel>(list, CacheKey, 60, 0, 0);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseModel> Insert(MaritalStatusModel model)
        {
            ResponseModel response = new ResponseModel();

            try
            {
                MaritalStatus md = new MaritalStatus();
                ...
                await _context.MaritalStatusRepository.AddAsync(md).ConfigureAwait(true);
        ...
        public async Task<ResponseModel> Update(MaritalStatusModel model)
        {
            ResponseModel response = new ResponseModel();

            try
            {
                MaritalStatus md = await _context.MaritalStatusRepository.FirstOrDefaultAsync(m => m.Id == model.Id);
                ...
                _context.MaritalStatusRepository.Update(md);
