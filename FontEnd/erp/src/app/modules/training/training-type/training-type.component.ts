...
export class TrainingTypeComponent implements OnInit {
  ...
  @ViewChild(TrainingTypeFormComponent) form: TrainingTypeFormComponent;
  ...
  private getList() {
    ...
    this.trainingTypeService.getList(filter).subscribe((response: ResponseModel) => {
  ...
  private deleteItem(itemId: number) {
    this.isLoading = true;
    const model = { id: itemId, action: FormActionStatus.Delete } as TrainingTypeViewModel;

    this.trainingTypeService.save(model).subscribe((response: ResponseModel) => {
      this.isLoading = false;
      if (response) {
        this.getList();
      }
