...
export class ProfessionalQualificationComponent implements OnInit {
  ...
  onDeleteClick(id: number) {
    if (this.isLoading !== true) {
      ...
      this.qualificationService.confirmDelete(id).subscribe((response: ResponseModel) => {
  ...
  private getList() {
    ...
    this.isLoading = true;
    this.qualificationService.getList(this.paging, this.searchText).subscribe((response: ResponseModel) => {
      if (response && response.responseStatus === ResponseStatus.success) {
