...
@Component({
  selector: 'app-hr-configuration-school-form',
...
export class SchoolFormComponent implements OnInit {
  ...
  constructor(
    ...
    ngOnInit(): void {
      let title = 'SCREEN.HR.CONFIGURATION.SCHOOL.FORM.TITLE_NEW';
      this.permission = this.schoolService.getPermission();
      ...
      if (this.dialogData) {
        if (this.dialogData.itemId) {
          title = 'SCREEN.HR.CONFIGURATION.SCHOOL.FORM.TITLE_EDIT';
    ...
    submitForm() {
      ...
      this.isLoading = true;
      const model = this.form.value as SchoolViewModel;
      model.action = this.formAction;

      this.schoolService.save(this.form.getRawValue(), this.formAction).subscribe((response: ResponseModel) => {
    ...
    private getItem(id: number) {
      this.isLoading = true;
      this.schoolService.item(id).subscribe((response: ResponseModel) => {
        if (response && response.responseStatus === ResponseStatus.success) {
    ...
    private setDataToForm(data: SchoolViewModel) {
