...
@Component({
  selector: 'app-hr-configuration-major',
...
export class MajorComponent implements OnInit {
  ...
  onDeleteClick(id: number, rowVersion: any) {
    if (this.isLoading === false) {
      ...
      this.majorService.confirmDelete(id, rowVersion).subscribe((response: ResponseModel) => {
  ...
  private getList() {
    this.isLoading = true;
    this.majorService.getList(this.paging, this.searchText).subscribe((response: ResponseModel) => {
      if (response && response.responseStatus === ResponseStatus.success) {
  ...
  private openModalForm(id?: number) {
    if (this.isLoading === false) {
      const modalRef = this.dialog.open(MajorFormComponent, {
        disableClose: true,
