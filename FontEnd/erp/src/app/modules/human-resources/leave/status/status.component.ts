...
export class LeaveStatusComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.permission = this.leaveStatusService.getPermission();
  ...
  onDeleteClick(id: number, rowVersion: any) {
    if (this.isLoading === false) {
      ...
      this.leaveStatusService.confirmDelete(id, rowVersion).subscribe((response: ResponseModel) => {
  ...
  private getList() {
    this.isLoading = true;
    this.leaveStatusService.getList(this.paging, this.searchText).subscribe((response: ResponseModel) => {
      if (response && response.responseStatus === ResponseStatus.success) {
  ...
  private openModalForm(id?: number) {
    if (this.isLoading === false) {
      const modalRef = this.dialog.open(LeaveStatusFormComponent, {
        disableClose: true,
